...
def orthonormalize(R):
    """Orthonormalize a 3x3 matrix using SVD."""
...
def is_valid_rotation_matrix(R):
    ...
    I = np.identity(3)
    is_orthogonal = np.allclose(should_be_identity, I, atol=1e-6)
    has_det_one = np.isclose(np.linalg.det(R), 1.0, atol=1e-6)
...
def interpolate_rotation(R1, R2, alpha):
    ...
    R_interp = r_interp.as_matrix()[0]
    

    # R_rounded = np.round(R_interp, decimals=4)

    R_ortho = orthonormalize(R_interp)


    if not is_valid_rotation_matrix(R_ortho):
        exit(0)
    else:
        print("no problem for via matrix")
...
if __name__ == '__main__':
    ...
    if arm_flag == 0: # left
        via_vector = np.array([0.15 , -0.1, 0.1]) # endeffector 좌표계 기준으로 생각해야
    else:
        via_vector = np.array([-0.15 ,-0.1, 0.1])
    via_matrix[:3, 3] = via_matrix[:3, 3] + via_matrix[:3, :3] @ via_vector
    # via_matrix = np.round(via_matrix, decimals=4)

    R_mid = interpolate_rotation(base2ee_lee[:3,:3], base2target[:3,:3], alpha=0.5)
    via_matrix[:3, :3] = R_mid
    # via_matrix[:3, :3] = base2target[:3, :3] 
