...
# 변수 정의
data_dir = "/home/vision/packages/FoundationPose/result_IROS_final_experiment/25_success_mustard_bottle"

CALIB_PATH_LEFT = "/home/vision/packages/calibration/data/data_0102_left/no_depth/hand_eye_v1_c2b_e2c_nominal_mean.yaml"
CALIB_PATH_RIGHT = "/home/vision/packages/calibration/data/data_0102_right/no_depth/hand_eye_v1_c2b_e2c_nominal_mean.yaml"
...
offset_up = 0.03    # 물체를 잡기 전에 얼마나 올려서 잡을지 # 아래서부터 위로 잡는 경우에 잡고 뒤로 빠지면서 물체가 선반 바닥과 충돌해서 콜리젼뜸
offset_back = 0.08  # 물체를 잡기 전에 얼마나 뒤로 빼서 잡을지 
via_up = 0.05        # via matrix얼마나 위로 
...
for s in device.sensors:
    ...
    if s.get_info(rs.camera_info.name) == 'RGB Camera':
        ...
        if s.supports(rs.option.enable_auto_exposure):
            print(f"Auto exposure enabled for RGB")
            s.set_option(rs.option.enable_auto_exposure,0)
            s.set_option(rs.option.gain,256.0)
            s.set_option(rs.option.exposure,256.0)
        ...
        if s.supports(rs.option.white_balance):
            print("✅ White balance supported")
            s.set_option(rs.option.enable_auto_white_balance, 1)
            # s.set_option(rs.option.white_balance, 3340)
        else:
...
filename = data_dir + f'/video_mppi_no_coll.mp4'
...
def send_and_wait_for_motion(q, socket_control, flag_code, ARM_flag, pipeline, control_mode,  motion_time):
    flag = bytes([int(flag_code)])
    time = np.array([motion_time])
    mode = np.array([control_mode])
    # print("time" , time)
    # print("mode" , mode)
    print("q_left ", q[0:6])
    print("q_left ", q[6:12])

    socket_control.send(flag + q.tobytes() + mode.tobytes() + time.tobytes())
...
def interpolate_xyzrpy(current_xyzrpy, target_xyzrpy, alpha=0.5, alpha_rot = 0.5):
    """
...
def execute_single_arm_grasp(task,next_skill):
    global gripper_left, gripper_right, home_xyzrpy
    arm = None
    ...
    half_depth_vector = np.array([0 , 0, -offset_back])
    ...
    # base2target_xyzrpy[2] = base2target_xyzrpy[2] + 0.05

    if arm == 'left':
        y_nom = base2target_xyzrpy[1] + 0.1 # 오른쪽으로 가는게 마이너스
        y = np.clip(y_nom, -0.15, 0.15)

        
    elif arm == 'right':
        y_nom = base2target_xyzrpy[1] - 0.1
        y = np.clip(y_nom, -0.15, 0.15)
    
    base2target_before_xyzrpy[2] = base2target_before_xyzrpy[2] + offset_up # 아래서부터 위로 잡는 경우에 잡고 뒤로 빠지면서 물체가 선반 바닥과 충돌해서 콜리젼뜸

    # 임시
    # base2target_before_xyzrpy[2] = base2target_before_xyzrpy[2] - 0.2 # 아래서부터 위로 잡는 경우에 잡고 뒤로 빠지면서 물체가 선반 바닥과 충돌해서 콜리젼뜸
    # base2target_xyzrpy[2] = base2target_xyzrpy[2] - 0.2 # 아래서부터 위로 잡는 경우에 잡고 뒤로 빠지면서 물체가 선반 바닥과 충돌해서 콜리젼뜸
    # base2target_before_xyzrpy[0] = base2target_before_xyzrpy[0] + 0.2 # 아래서부터 위로 잡는 경우에 잡고 뒤로 빠지면서 물체가 선반 바닥과 충돌해서 콜리젼뜸
    # base2target_xyzrpy[0] = base2target_xyzrpy[0] + 0.2 # 아래서부터 위로 잡는 경우에 잡고 뒤로 빠지면서 물체가 선반 바닥과 충돌해서 콜리젼뜸
    # 로봇에 맞게 좌표 변경 이전에 몇센치 뒤에 있다가 빠지는 지 
    via_matrix_xyzrpy = interpolate_xyzrpy(current_xyzrpw, base2target_xyzrpy, alpha=0.5)
    if (arm_flag) == "left": # left
        print("[LEFT]")
        via_position = np.array([via_matrix_xyzrpy[0] , y, base2target_xyzrpy[2]+via_up]) # endeffector 좌표계 기준으로 생각해야
        # 0.39706217 -0.08047105  0.40784926
    elif arm_flag == 'right':
        print("[RIGHT]")
        via_position = np.array([via_matrix_xyzrpy[0] , y, base2target_xyzrpy[2]+via_up])
    else:
        print("❌ ARM Selection Error ❌")
    ...
    if home_xyzrpy is None:
        home_xyzrpy = current_xyzrpw
    ...
    # send_and_wait_for_motion(via_matrix_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, ARM_flag, pipeline,3.0, motion_time)

    print("via_matrix_xyzrpy ", via_matrix_xyzrpy)

    print("base2target_before_xyzrpy ", base2target_before_xyzrpy)
    print("base2target_xyzrpy ", base2target_xyzrpy)
    ...
    for i in range(1, n + 1):
        a = i / n
        l = interpolate_xyzrpy(current_xyzrpw[0:6],  via_matrix_xyzrpy[0:6],  alpha=float(a))
        r = interpolate_xyzrpy(current_xyzrpw[6:12], via_matrix_xyzrpy[6:12], alpha=float(a))
        wp = np.concatenate([l, r])
        motion_time = compute_motion_time(current_xyzrpw, base2target_before_xyzrpy, arm_flag)
        send_and_wait_for_motion(wp, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline,3.0, motion_time)



    # before matrix로 이동
    motion_time = compute_motion_time(via_matrix_xyzrpy, base2target_before_xyzrpy,arm_flag)
    send_and_wait_for_motion(base2target_before_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, ARM_flag, pipeline, 3.0, motion_time)



    # target으로 이동
    motion_time = compute_motion_time(base2target_before_xyzrpy, base2target_xyzrpy,arm_flag)
    send_and_wait_for_motion(base2target_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, ARM_flag, pipeline, 3.0, motion_time)
    ...
    for i in range(1, n + 1):
        a = i / n
        l = interpolate_xyzrpy(base2target_before_xyzrpy[0:6],  via_matrix_xyzrpy[0:6],  alpha=float(a))
        r = interpolate_xyzrpy(base2target_before_xyzrpy[6:12], via_matrix_xyzrpy[6:12], alpha=float(a))
        wp = np.concatenate([l, r])
        motion_time = compute_motion_time(base2target_before_xyzrpy, home_xyzrpy, arm_flag)
        send_and_wait_for_motion(wp, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline,3.0, motion_time)

    # current로 이동
    if True: #next_skill is None or 'place' not in next_skill:
        motion_time = compute_motion_time(via_matrix_xyzrpy, home_xyzrpy, arm_flag)
        send_and_wait_for_motion(home_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, ARM_flag, pipeline,3.0, motion_time)
...
def execute_bimanual_grasp(task ,next_skill):
    ...
    # base2target_l[0] = base2target_l[0] - 0.2 # 아래서부터 위로 잡는 경우에 잡고 뒤로 빠지면서 물체가 선반 바닥과 충돌해서 콜리젼뜸
    # base2target_before_l[0] = base2target_before_l[0] - 0.2 # 아래서부터 위로 잡는 경우에 잡고 뒤로 빠지면서 물체가 선반 바닥과 충돌해서 콜리젼뜸
    ...
    base2target_before_xyzrpy_l[2] = base2target_before_xyzrpy_l[2] + offset_up
    base2target_before_xyzrpy_r[2] = base2target_before_xyzrpy_r[2] + offset_up
    ...
    print("base2target_xyzrpy_l ",base2target_xyzrpy_l)
    print("base2target_xyzrpy_r", base2target_xyzrpy_r)
    ...
    yL = np.clip(yL_nom, -0.1, 0.1) # 오른쪽으로 갈수록 플러스
    yR = np.clip(yR_nom, -0.1, 0.1)
    ...
    via_position_l = np.array([via_matrix_xyzrpy_l[0] , yL , base2target_xyzrpy_l[2]+via_up]) # endeffector 좌표계 기준으로 생각해야
    via_position_r = np.array([via_matrix_xyzrpy_r[0] , yR , base2target_xyzrpy_r[2]+via_up])
    ...
    # send_and_wait_for_motion(via_matrix_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline,3.0, motion_time)
    ...
    for i in range(1, n + 1):
        ...
        r = interpolate_xyzrpy(current_xyzrpw[6:12], via_matrix_xyzrpy[6:12], alpha=float(a))
        wp = np.concatenate([l, r])
        send_and_wait_for_motion(wp, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline,3.0, 0.0)

    print("GO BEFORE")
    # before matrix로 이동
    motion_time = compute_motion_time(via_matrix_xyzrpy, base2target_before_xyzrpy,'both')
    send_and_wait_for_motion(base2target_before_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline, 3.0, motion_time)
    
    # target으로 이동
    motion_time = compute_motion_time(base2target_before_xyzrpy, base2target_xyzrpy,'both')
    send_and_wait_for_motion(base2target_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline, 3.0, motion_time)
    ...
    for i in range(1, n + 1):
        a = i / n
        l = interpolate_xyzrpy(base2target_xyzrpy[0:6],  via_matrix_xyzrpy[0:6],  alpha=float(a))
        r = interpolate_xyzrpy(base2target_xyzrpy[6:12], via_matrix_xyzrpy[6:12], alpha=float(a))
        wp = np.concatenate([l, r])
        send_and_wait_for_motion(wp, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline,3.0, 0.0)
    ...
    # for i in range(1, n + 1):
    #     a = i / n
    #     l = interpolate_xyzrpy(via_matrix_xyzrpy[0:6],  home_xyzrpy[0:6],  alpha=float(a))
    #     r = interpolate_xyzrpy(via_matrix_xyzrpy[6:12], home_xyzrpy[6:12], alpha=float(a))
    #     wp = np.concatenate([l, r])
    #     send_and_wait_for_motion(wp, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline,3.0, 0.0)
    ...
    # current로 이동
        # current로 이동
    if True: # next_skill is None or 'place' not in next_skill:
        motion_time = compute_motion_time(via_matrix_xyzrpy, home_xyzrpy, 'both')
        send_and_wait_for_motion(home_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline, 3.0, motion_time)
...
def execute_single_arm_place(task):
    ...
    send_and_wait_for_motion(end_matrix_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, ARM_flag, pipeline,3.0, motion_time)
    ...
    # current로 이동
    motion_time = compute_motion_time(end_matrix_xyzrpy, home_xyzrpy, arm)
    send_and_wait_for_motion(home_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, ARM_flag, pipeline,3.0, motion_time)
...
def execute_bimanual_place(task):
    ...
    end_position_l = np.array([0.3 , 0.0, 0.15]) #np.array([0.45 , 0.0, 0.15])
    end_rotation = np.array([-3.13, -0.00,  0.00])
    # if object_name == "the red box.":
    ...
    end_position_r = np.array([0.3 , 0.0, 0.15]) #np.array([0.45 , 0.0 , 0.15])
    # if object_name == "the red box.":
    #     end_position = np.array([0.3 , 0.0, 0.20])
    # elif object_name == "the red can." or object_name == "the white can.":
    #     end_position = np.array([0.3 , 0.0, 0.14])
    ...
    # end matrix로 이동
    motion_time = compute_motion_time(current_xyzrpw, end_matrix_xyzrpy, arm)
    send_and_wait_for_motion(end_matrix_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline,3.0, motion_time)
...
if __name__ == '__main__':
    ...
    with open(os.path.join(data_dir , "gpt_planning_output.json"), "r") as f:
      data = json.load(f)
    ...
    # ✅ 전체 execution 시간 측정 종료(예외여도 실행됨)
    t_end = time.perf_counter()
    total_sec = t_end - t_start
    print(f"\n==== ⏱️ Total execution time: {total_sec:.3f} s ({total_sec/60:.2f} min) ====\n")
