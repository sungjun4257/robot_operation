...
# 변수 정의
data_dir = "/home/vision/packages/FoundationPose/result_IROS_final_experiment/1_success_box_apple"
...
filename = data_dir + f'/video_clik.mp4'
...
def compute_motion_time(current_xyzrpy, target_xyzrpy, arm_flag= None,
                        max_linear_speed=1.0, max_angular_speed=0.5):
    """
...
def execute_single_arm_grasp(task,next_skill):
    ...
    for i in range(1, n + 1):
        ...
        send_and_wait_for_motion(wp, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline,1.0, motion_time)



    # before matrix로 이동
    motion_time = compute_motion_time(via_matrix_xyzrpy, base2target_before_xyzrpy,arm_flag)
    send_and_wait_for_motion(base2target_before_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, ARM_flag, pipeline, 1.0, motion_time)



    # target으로 이동
    motion_time = compute_motion_time(base2target_before_xyzrpy, base2target_xyzrpy,arm_flag)
    send_and_wait_for_motion(base2target_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, ARM_flag, pipeline, 1.0, motion_time)
    ...
    if True: #next_skill is None or 'place' not in next_skill:
        ...
        send_and_wait_for_motion(home_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, ARM_flag, pipeline,1.0, motion_time)
...
def execute_bimanual_grasp(task ,next_skill):
    ...
    send_and_wait_for_motion(base2target_before_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline, 1.0, motion_time)
    
    # target으로 이동
    motion_time = compute_motion_time(base2target_before_xyzrpy, base2target_xyzrpy,'both')
    send_and_wait_for_motion(base2target_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline, 1.0, motion_time)
    ...
    for i in range(1, n + 1):
        ...
        wp = np.concatenate([l, r])
        send_and_wait_for_motion(wp, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline,1.0, 0.0)
    ...
    if True: # next_skill is None or 'place' not in next_skill:
        ...
        send_and_wait_for_motion(home_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline, 1.0, motion_time)
...
def execute_single_arm_place(task):
    ...
    send_and_wait_for_motion(end_matrix_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, ARM_flag, pipeline,1.0, motion_time)
...
def execute_bimanual_place(task):
    ...
    # end matrix로 이동
    motion_time = compute_motion_time(current_xyzrpw, end_matrix_xyzrpy, arm)
    send_and_wait_for_motion(end_matrix_xyzrpy, socket_control, NetProto.MOVE_ARM_BY_XYZRPY, 'both', pipeline,1.0, motion_time)
